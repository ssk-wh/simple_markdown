...
- **[INV-TAB-SLEEP-FIFO]** 已加载 Tab 数超过 `m_tabSleepThreshold`（默认 20）时，按 `openOrder`（创建时分配的单调序号）升序选最早的 Tab 调用 `dormantTab` 休眠——卸载 editor 内容（`Document::remove(0, length) + setModified(false)`）+ 清预览（`PreviewWidget::updateAst(nullptr)`），设 `lazyPending=true` + `lazyFilePath=fp`。复用现有 lazy 机制：`onTabChanged` 切到 lazy Tab 时自动 `loadFromFile + parseNow` 唤醒。豁免对象：当前活跃 Tab、未保存修改的 Tab、文件路径为空（无源可恢复）的 Tab。触发时机：`onTabChanged` 末尾（包含切换 + 新建后的全部路径）。
