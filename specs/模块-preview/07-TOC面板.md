...
`TocPanel::preferredWidth()` **只在 `setEntries` / `setCurrentFileKey` 时**重新计算并以 `preferredWidthChanged(int)` 信号通知宿主（即：首次加载文档 / 切换 Tab 时）。折叠/展开章节（`toggleCollapse`）**不**触发重算、**不**发射该信号——宽度稳定优先于内容自适应（见 T-WIDTH-NO-JITTER）。宿主（`MainWindow`）未检测到用户手动拖拽 splitter 时，自动套用该宽度。
...
- **T-COLLAPSE-5** 初次加载含持久化折叠态的文档时，折叠项不计入宽度公式（避免初始宽度过宽）
- **T-WIDTH-NO-JITTER** 折叠/展开过程中 `preferredWidthChanged` 不发送，splitter sizes 保持不变
