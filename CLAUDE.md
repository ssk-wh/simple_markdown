...
8. **每个 bug 修复必须配回归单元测试**：开发过程中出现的任何 bug，修复时必须同步新增（或扩展）一条能复现该问题的单元测试，作为防回归门禁。测试嵌入对应 Spec 的 T 编号（见第 4 条），无对应 T 条目则先补 Spec。**禁止「只改代码不加测试」**——无法写出复现测试时（如纯视觉/外部环境问题），必须在 plan 的「验证」段说明原因及替代验证方式（如像素截图对比）。历史已修 bug 若缺对应单测，发现时应顺手补齐。
9. **单元测试必须全面覆盖核心函数和边界场景**：每个类/模块的核心公开函数必须有对应的单元测试，包含正常路径、边界值（空输入、极值、越界）、异常路径（不合法参数、错误状态）。测试按模块目录组织（`tests/core/`、`tests/parser/`、`tests/preview/` 等），与 Spec 第 7 节「验收条件」的 T 编号对齐。纯 GUI/异步/窗口操作等难以用单测覆盖的场景，在对应 Spec 或 plan 中记录替代验证方式（手动测试、截图对比、集成测试等），并用 `#pragma note(manual-test)` 或同类标记告知后继开发者。
...
- **CHANGELOG 版本号必须与 git tag 严格一致**。每次修改 CHANGELOG 时，确保 `## [x.y.z]` 中的版本号与将要打的 tag 相同。
- 每次有新修改时，版本号递增一位（patch）。但如果最新的版本号**尚未推送**（即仓库中不存在对应 tag），则直接在该已有条目中追加改动，不创建新版本号；**已推送**（远程已有 tag）则必须新增版本号。
- 打 tag 前先确认 CHANGELOG 中的最新版本号与 tag 一致，不一致先修正再打 tag。
- Tag 打好后**禁止移动**，有新的改动走下一个递增版本号。
